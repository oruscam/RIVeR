...
export const VideoPlayerButtons = ({ videoRef, control, setControl }: VideoPlayerButtonsProps) => {
    ...
    return (
        <div className='video-player-buttons'>
            <button className='video-button' onClick={handlePreviuos}> <Icon path={previous}/></button>
            <button className='video-button' onClick={handlePlay}> <Icon path={control.play ? pause : play}/></button>
            <button className='video-button' onClick={handleNext}> <Icon path={next}/></button>
        </div>
    );
};
