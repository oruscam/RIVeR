...
export const HardModeCoordinates = ({modeName, factor}: {modeName: string, factor: {x:number, y:number}}) => {
    ...
    useEffect(() => {
      if(points.length !== 0){
        ...
        setValue(`${modeName}-X_${pointName1}`, points[1].x, {shouldValidate: true})
        setValue(`${modeName}-Y_${pointName1}`, points[1].y, {shouldValidate: true})
        setValue(`${modeName}-X_${pointName2}`, points[0].x, {shouldValidate: true})
        setValue(`${modeName}-Y_${pointName2}`, points[0].y, {shouldValidate: true})
      }
    }, [points])
    ...
    //     if ( parseFloat(x1) !== localPoints[1].x || parseFloat(y1) !== localPoints[1].y || parseFloat(x2) !== localPoints[0].x || parseFloat(y2) !== localPoints[0].y){ 
    //       const newPoint = [{x: parseFloat(x2), y: parseFloat(y2)}, {x: parseFloat(x1), y: parseFloat(y1)}]
    //       onSetPoints(newPoint, 1, 1)
    //     }
    // }
    // },[x1, x2, y1, y2])

    const handleKeyDown = ( event, nextFieldId ) => {
      if(event.key === 'Enter' && localPoints){
        ...
        switch (event.target.id) {
          case `X_${pointName1}`:
            if( value !== localPoints[1].x){
              console.log("cambio x1")
              setLocalPoints([{x: value, y: localPoints[1].y}, {x: localPoints[0].x, y: localPoints[0].y}])
              onSetPoints([{x: value, y: localPoints[1].y}, {x: localPoints[0].x, y: localPoints[0].y}], 1, 1)
            }
            break;
          
          case `Y_${pointName1}`:
            if( value !== localPoints[1].y){
              console.log("cambio y1")
            }
            break;
    
          case `X_${pointName2}`:
            if( value !== localPoints[0].x){
              console.log("cambio x2")
            }
            break;

          case `Y_${pointName2}`:
            if( value !== localPoints[0].y){
              console.log("cambio y2")
            }
            break;
    ...
    return (
        <div className='hard-mode' id={`${modeName}-HARD_MODE`}>
            ...
            <input type='number' step="any" className='input-field' {...register(`${modeName}-X_${pointName1}`)} defaultValue={0} id={`X_${pointName1}`} onKeyDown={(event) => handleKeyDown(event, `Y_${pointName1}`)}/>
            <label className='read-only red' htmlFor={`Y_${pointName1}`}> {`Y ${pointName1}`}</label>
            <input type='number' step="any" className='input-field' {...register(`${modeName}-Y_${pointName1}`)} defaultValue={0} id={`Y_${pointName1}`} onKeyDown={(event) => handleKeyDown(event, `X_${pointName2}`)}/>

            <label className='read-only green' htmlFor={`X_${pointName2}`}> {`X ${pointName2}`}</label> 
            <input type='number' step="any" className='input-field' {...register(`${modeName}-X_${pointName2}`)} defaultValue={0} id={`X_${pointName2}`} onKeyDown={(event) => handleKeyDown(event, `Y_${pointName2}`)}/>
            <label className='read-only green' htmlFor={`Y_${pointName2}`}> {`Y ${pointName2}`}</label>
            <input type='number' step="any" className='input-field' {...register(`${modeName}-Y_${pointName2}`)} defaultValue={0} id={`Y_${pointName2}`} onKeyDown={(event) => handleKeyDown(event, 'wizard-next')}/>
        </div>
  )
}
