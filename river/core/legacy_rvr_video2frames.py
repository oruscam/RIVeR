...
def rvr_video2frames(video_path, frames_dir, overwrite=False,start_frame_number=-1, end_frame_number=-1, every=1, chunk_size=1000):
    ...
    # print(os.path.join(frames_dir, video_filename))
    ...
    frame_chunks = [[i, i+chunk_size] for i in range(start_frame_number, end_frame_number, chunk_size)]  # split the frames into chunk lists
    frame_chunks[-1][-1] = min(frame_chunks[-1][-1], end_frame_number)  # make sure last chunk has correct end frame, also handles case chunk_size < total
    ...
    with ThreadPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        ...
        for i, ff in enumerate(as_completed(futures)):  # as each process completes
