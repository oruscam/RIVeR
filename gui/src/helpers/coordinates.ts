...
function transformPixelToRealWorld(
  x_pix: number,
  y_pix: number,
  ...
  const normalizedVector = realWorldVector.map(
    (value) => value / realWorldVector[2],
  );
...
function transformRealWorldToPixel(
  x_rw: number,
  y_rw: number,
  transformationMatrix: number[][],
): number[] {
 ...
 */
function getTransformationFromCameraMatrix(
  cameraMatrix: number[][],
  z_level: number,
): number[][] {
