...
export const FramesResolution = ({ active }: { active: boolean }) => {
    ...
    return (
        <div className={`video-resolution ${active ? '' : 'hidden'}`} id='video-resolution'>
            ...
            <button className={`wizard-button mt-1 ${ factor === 1 ? 'wizard-button-active' : ''}`} type="button" id="1" onClick={onClickResolution}>{value1}</button>
            <button className={`wizard-button mt-1 ${ factor === 0.5 ? 'wizard-button-active' : ''}`} type="button" id="0.5" onClick={onClickResolution}>{value2}</button>
            <button className={`wizard-button mt-1 ${ factor === 0.25 ? 'wizard-button-active' : ''}`} type="button" id="0.25" onClick={onClickResolution}>{value3}</button>
        </div>
    )
}