  ...
  return (
    <div className="lwrap" ref={ref}>
      ...
      <div className={`ldrop ${open ? "open" : ""}`}>
        {THEMES.map(t => (
          <div key={t.value} className={`litem ${t.value === theme ? "sel" : ""}`}
            onClick={() => { onSetTheme(t.value); setOpen(false) }}>
            <span style={{ fontSize: 17 }}>{t.icon}</span>
            <span>{t.label}</span>
            {t.value === theme && <span style={{ marginLeft: "auto", display: "flex" }}>{Icons.Check("var(--success-color)")}</span>}
          </div>
        ))}
      </div>
    </div>
