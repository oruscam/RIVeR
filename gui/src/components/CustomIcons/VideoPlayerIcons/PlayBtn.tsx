...
import { useProjectSlice } from "../../../hooks/useProjectSlice";
...
export function PlayBtn({ videoRef, control, setControl }: PlayBtnProps) {
    ...
    return (
        <button type="button" className={`ib ${control.play ? "" : ""}`} onClick={control.play ? handlePause : handlePlay}>
            {rpl}
            <span className="cl show">
                {control.play ? Icons.Pause("currentColor") : Icons.Play("currentColor")}
            </span>
        </button>
