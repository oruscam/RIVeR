...
export const useMatrixSlice = () => {
  ...
  const { obliquePoints, hasChanged, ipcam, pixelSize, isBackendWorking } =
    useSelector((state: RootState) => state.matrix);
  const { t } = useTranslation();

  const onSetObliqueCoordinates = (
    points: Point[],
  ...
  const onChangeObliqueCoordinates = (canvasPoint: CanvasPoint | null) => {
    if (canvasPoint) {
    ...
    if (isDistancesLoaded) {
      dispatch(
        setObliquePoints({
          ...
          distances: { d12: 0, d13: 0, d23: 0, d24: 0, d34: 0, d41: 0 },
          solution: undefined,
        }),
      );
      return;
  ...
  const onGetTransformationMatrix = async (
    type: "uav" | "ipcam" | "oblique",
    formDistances?: FieldValues,
  ) => {
    ...
    if (type === "oblique" && formDistances) {
      ...
      for (const key in newDistances) {
        if (
          newDistances[key as keyof typeof distances] !==
      ...
      try {
        const {
          ...
          error,
        } = await ipcRenderer.invoke("set-control-points", {
          coordinates,
        ...
        const rwCoordinates = coordinates.map((point) => {
          const cord1 = transformPixelToRealWorld(
            point.x,
            point.y,
            obliqueMatrix,
          );
        ...
        dispatch(
          setTransformationMatrix({ transformationMatrix: obliqueMatrix }),
        );
        dispatch(
          setObliquePoints({
            ...obliquePoints,
    ...
    if (type === "uav") {
      ...
      try {
        const { uavMatrix, transformed_image_path, output_resolution, extent } =
          await ipcRenderer.invoke("set-pixel-size", args);

        const secondPoint = transformPixelToRealWorld(
          dirPoints[1].x,
          dirPoints[1].y,
          uavMatrix,
        );
  ...
  const onChangeIpcamPointSelected = ({
    index,
    rowsIndex,
  }: {
    index?: number;
    ...
    if (index !== undefined && index >= 0) {
      ...
      if (rowsIndex !== ipcam.importedPoints.filter((v) => v.selected).length) {
        const newPoints = ipcam.importedPoints.map((point) => {
        ...
        dispatch(
          setIpcamPoints({
            points: newPoints,
            path: undefined,
            counter: value ? ipcam.importedPoints.length : 0,
          }),
        );
      }
    }
  ...
  const setIpcamPointPixelCoordinates = ({
    index,
    imageSize,
    point,
    clickIcon,
  }: setIpcamPointPixelCoordinatesInterface) => {
    ...
    try {
      ...
      dispatch(
        setIpcamCameraSolution({
          ...data,
          orthoImagePath:
            filePrefix + data.orthoImagePath + `?t=${new Date().getTime()}`,
          mode: mode,
          numPoints: numPoints,
        }),
      );
  ...
  const onUpdatePixelSize = (value: UpdatePixelSize) => {
    ...
    if (value.pixelSize !== undefined) {
      if (
        (dirPoints[0] === DEFAULT_POINTS[0] &&
          dirPoints[1] === DEFAULT_POINTS[1]) ||
        dirPoints.length === 0
      ) {
        const newDirPoints = getLinesCoordinates(
          value.imageWidth!,
          value.imageHeight!,
        );
